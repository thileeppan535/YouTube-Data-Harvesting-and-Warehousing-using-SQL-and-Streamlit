...
if selected == "Data collection and upload":
    ...
    if st.button("Upload to MYSQL database"): # upload the youtube retrieved data into MYSQL database
        ...
        with st.spinner('Upload in progress...'):
            try:
                mycursor.execute("DELETE FROM channel WHERE channel_id = %s", (channel_ID,))
                mycursor.execute("DELETE FROM playlist WHERE channel_id = %s", (channel_ID,))
                mycursor.execute("DELETE FROM videos WHERE channel_id = %s", (channel_ID,))
                mycursor.execute("DELETE FROM comments WHERE video_id IN (%s)" % ','.join(["%s"] * len(video_IDS)), tuple(video_IDS))
